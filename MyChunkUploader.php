...
class MyChunkUploader {
	...
	function __construct( $working_dir = null ) {
		...
		defined( 'UPLOADER_CHUNK_SIGNATURE' ) || define( 'UPLOADER_CHUNK_SIGNATURE', $class_header ); // the class signature
		$prefix = UPLOADER_CHUNK_SIGNATURE;
		
		defined( 'UPLOADER_WAIT_HEADER' ) || define( 'UPLOADER_WAIT_HEADER', $prefix . '-Wait' ); // the wait header name
		defined( 'UPLOADER_TYPE_HEADER' ) || define( 'UPLOADER_TYPE_HEADER', $prefix . '-Type' ); // the content-type header name
		defined( 'UPLOADER_NONCE_HEADER' ) || define( 'UPLOADER_NONCE_HEADER', $prefix . '-Security-Nonce' ); // the nonce header name
		defined( 'UPLOADER_RAW_POST_HEADER' ) || define( 'UPLOADER_RAW_POST_HEADER', $prefix . '-Raw-Post' ); // the raw post option header name
		defined( 'UPLOADER_ABORT_HEADER' ) || define( 'UPLOADER_ABORT_HEADER', $prefix . '-Abort' ); // the abort flag header name
		defined( 'UPLOADER_TIMEOUT_HEADER' ) || define( 'UPLOADER_TIMEOUT_HEADER', $prefix . '-Timeout' ); // the upload timeout header name
		
		defined( 'UPLOADER_RANGE_HEADER' ) || define( 'UPLOADER_RANGE_HEADER', 'Content-Range' ); // the range header name
		defined( 'UPLOADER_FILENAME_HEADER' ) || define( 'UPLOADER_FILENAME_HEADER', 'Content-Disposition' ); // the filename header name
		
		defined( 'UPLOADER_TIMEOUT' ) || define( 'UPLOADER_TIMEOUT', 3600 ); // how long to wait to merge files
		...
		if ( defined( 'UPLOADER_VERIFY_NONCE_CALLBACK' ) && is_callable( UPLOADER_VERIFY_NONCE_CALLBACK ) ) {
			$this->on_chk_nonce = UPLOADER_VERIFY_NONCE_CALLBACK;
			$this->_require_nonce = defined( 'UPLOADER_REQUIRES_NONCE' ) && UPLOADER_REQUIRES_NONCE;
		}
